...
def create_staff_member(
    staff_member: models.StaffMemberCreate, session: Session = Depends(get_session)
):
...
def read_staff_member(staff_member_id: UUID, session: Session = Depends(get_session)):
    db_staff_member = crud.get_staff_member(
        session=session, staff_member_id=staff_member_id
    )
...
def update_staff_member(
    ...
    return crud.update_staff_member(
        session=session,
        db_staff_member=db_staff_member,
        staff_member_update=staff_member_update,
    )
