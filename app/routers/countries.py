...
def create_country(
    country: models.CountryCreate, session: Session = Depends(get_session)
):
    db_country = crud.get_country(session, country_code=country.alpha_2_code)
    if db_country:
        raise HTTPException(
            status_code=400, detail="Country with this code already exists"
        )
...
def update_country(
    country_code: str,
    country_update: models.CountryUpdate,
    ...
    return crud.update_country(
        session=session, db_country=db_country, country_update=country_update
    )
